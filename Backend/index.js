...
io.on("connection", (socket) => {
  ...
  // Fix: Handling user leaving properly
  socket.on("leaveRoom", () => {
    if (currentRoom && currentUser) {
      rooms.get(currentRoom)?.users.delete(currentUser);
      io.to(currentRoom).emit("userJoined", Array.from(rooms.get(currentRoom)?.users || []));
      socket.leave(currentRoom);
      console.log(`User ${currentUser} left room ${currentRoom}`); // Debug log
      currentRoom = null;
      currentUser = null;
    }
  });

  // Fix: Handling typing events correctly
  socket.on("typing", ({ roomId, userName }) => {
    if (!roomId || !userName) return;
    console.log(`User ${userName} is typing in room ${roomId}`); // Debug log
    io.to(roomId).emit("userTyping", { user: userName }); // Broadcast to room
  });

  // Fix: Handling language change properly
  socket.on("languageChange", ({ roomId, language }) => {
    if (!roomId || !language) return;
    console.log(`Language changed to ${language} in room ${roomId}`); // Debug log
